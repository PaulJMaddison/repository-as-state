...
\caption{Conceptual Repository-as-State architecture. The durable object is accepted project state; reasoning and execution processes may be replaced between validated transitions. No empirical performance is implied.}
...
After hostile review of current prior work, paper v0.1 claims the following narrower contributions:
...
\begin{enumerate}
    ...
    \item explicit measurement of repository rediscovery as a possible negative result rather than treating reconstruction as free;
    \item a structured, non-chain-of-thought reconstruction probe that separates understanding of accepted state from downstream implementation success;
    \item semantic-state ablation at accepted task boundaries to study which repository artefacts have marginal continuity value;
    \item a broader lifecycle framing in which reasoning persistence, execution state, and operational privilege can be varied independently, while treating tiered execution and security benefits as secondary hypotheses rather than core novelty.
\end{enumerate}

The paper does not claim empirical findings. P0 has not been executed. P0 is intended as a methodology pilot and initial case study, not a proof of general superiority.
